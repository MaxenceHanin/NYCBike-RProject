...
m02 <-
  read_csv("~/INSA/5e année/NYCBike-RProject-perso/dataset/2018/201802.csv")
m03 <-
  read_csv("~/INSA/5e année/NYCBike-RProject-perso/dataset/2018/201803.csv")
m04 <-
  read_csv("~/INSA/5e année/NYCBike-RProject-perso/dataset/2018/201804.csv")
m05 <-
  read_csv("~/INSA/5e année/NYCBike-RProject-perso/dataset/2018/201805.csv")
m06 <-
  read_csv("~/INSA/5e année/NYCBike-RProject-perso/dataset/2018/201806.csv")
m07 <-
  read_csv("~/INSA/5e année/NYCBike-RProject-perso/dataset/2018/201807.csv")
m08 <-
  read_csv("~/INSA/5e année/NYCBike-RProject-perso/dataset/2018/201808.csv")
m09 <-
  read_csv("~/INSA/5e année/NYCBike-RProject-perso/dataset/2018/201809.csv")
m10 <-
  read_csv("~/INSA/5e année/NYCBike-RProject-perso/dataset/2018/201810.csv")
m11 <-
  read_csv("~/INSA/5e année/NYCBike-RProject-perso/dataset/2018/201811.csv")
m12 <-
...
nbr_trip_month <- function(data_table, d, m) {
  ...
  data_table$starttime <-
    d + as.integer(as.character(substr(data_table$starttime, 9, 10)))
  data_table$bikeid <- 1
  temp <-
    data.table(
      data.frame(
        day = data_table$starttime,
        number = data_table$bikeid,
        month = data_table$stoptime
      )
    )
...
age <-
  2018 - m01$`birth year`[which(m01$gender > 0 &
                                  m01$`birth year` > 1940)]
start_lat <-
  m01$`start station latitude`[which(m01$gender > 0 &
                                       m01$`birth year` > 1940)]
start_long <-
  m01$`start station longitude`[which(m01$gender > 0 &
                                        m01$`birth year` > 1940)]
stop_lat <-
  m01$`end station latitude`[which(m01$gender > 0 &
                                     m01$`birth year` > 1940)]
stop_long <-
...
between_20_30 = c(val_max, 0, sum(age_group$freq[which(age_group$x > 20 &
                                                         age_group$x < 30)]))
between_30_40 = c(val_max, 0, sum(age_group$freq[which(age_group$x > 30 &
                                                         age_group$x < 40)]))
between_40_50 = c(val_max, 0, sum(age_group$freq[which(age_group$x > 40 &
                                                         age_group$x < 50)]))
between_50_60 = c(val_max, 0, sum(age_group$freq[which(age_group$x > 50 &
                                                         age_group$x < 60)]))
...
start_lat8 <-
  m08$`start station latitude`[which(m08$gender > 0 &
                                       m08$`birth year` > 1940)]
start_long8 <-
  m08$`start station longitude`[which(m08$gender > 0 &
                                        m08$`birth year` > 1940)]
stop_lat8 <-
  m08$`end station latitude`[which(m08$gender > 0 &
                                     m08$`birth year` > 1940)]
stop_long8 <-
  m08$`end station longitude`[which(m08$gender > 0 &
                                      m08$`birth year` > 1940)]
distance8 <-
...
top_10_data <-
  top_10_temp[, .(distance = sum(na.omit(distance))), by = velo]
